...
public class HttpClientUtil2 {
	 ...
	 * @return httpcode
	 */
	public static String invokeGet_1(String url) {
		int responseCode = -1;
	...
	public static Token invokeGet_0(String url) {
		int responseCode = -1;
		Token token = null;
		RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(connectTimeout)
				.setConnectTimeout(connectTimeout).setConnectionRequestTimeout(connectTimeout).build();
		...
		try {
			...
			try {
				...
				logger.info("entity result is:" + string);
				token = JSON.parseObject(string, Token.class);
			} catch (Exception e) {
				logger.error(String.format("get response error, url:%s", url), e);
			} finally {
				if (response != null) {
